...
try:
    algorithm = header.decode('utf-8') # Mendapatkan informasi tentang algoritma enkripsi yang digunakan dari pengirim
except UnicodeDecodeError:
    algorithm = header.decode('latin-1')
...
print(header)  # Cetak data header untuk memastikan bahwa data yang diterima sudah benar
algorithm = header.decode('utf-8')  # Mendapatkan informasi tentang algoritma enkripsi yang digunakan dari pengirim

 # Menerima informasi algoritma enkripsi dari header pesan
algorithm = header.decode('utf-8') # Mendapatkan informasi tentang algoritma enkripsi yang digunakan dari pengirim
try:
    header = f.read(HEADER_SIZE).encode('latin-1')
    algorithm = header.decode('utf-8')
except UnicodeDecodeError:
    algorithm = header.decode('latin-1')
...
conn.shutdown(socket.SHUT_RDWR) # Menutup koneksi secara bersih
conn.close()
sock.close()
...
if algorithm == 'AES':
    ...
    print("Algoritma enkripsi tidak dikenali")
    plaintext = b''  # Atau dapat diberikan tindakan yang sesuai jika algoritma tidak dikenali
